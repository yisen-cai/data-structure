...
template <class T>
void QueueByStackWay1<T>::enqueue(T element) {
...
template <class T>
T QueueByStackWay1<T>::dequeue() {
  return stack1.pop();
}

template <class T>
class QueueByStackWay2 {
  LinkedListStack<T> stack1;
  LinkedListStack<T> stack2;
...
template <class T>
void QueueByStackWay2<T>::enqueue(T data) {
  stack1.push(data);
}
template <class T>
T QueueByStackWay2<T>::dequeue() {
  while (!stack1.isEmpty()) {
    stack2.push(stack1.pop());
  }
  T result = stack2.pop();
